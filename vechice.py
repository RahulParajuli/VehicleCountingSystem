...
while True:
    ...
    for (x,y) in detect:
        ...
        detect.remove((x,y))
        print("vehicle passed : "+str(counter))

    cv2.putText(frame1, "vehicle passed :"+str(counter),(450,70),cv2.FONT_HERSHEY_SIMPLEX, 2 , (0,0,255),5) 


    
    # cv2.imshow('Vehicle Detector', frame1)
    cv2.imshow('CCTV camera 1', frame1)
    if cv2.waitKey(1)== 13:
        break
